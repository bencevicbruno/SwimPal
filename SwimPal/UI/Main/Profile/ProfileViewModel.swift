//
//  ProfileViewModel.swift
//  SwimPal
//
//  Created by Bruno Benčević on 02.06.2022..
//

import Combine
import SwiftUI

final class ProfileViewModel: ObservableObject {
    
    var onGoToStatistics: EmptyCallback?
    var onGoToAchievements: EmptyCallback?
    var onGoToSettings: EmptyCallback?
    var onGoToAbout: EmptyCallback?
    var onLoggedOut: EmptyCallback?
    
    @Published var confirmationData: ConfirmationData?
    
    @Dependency var persistenceService: PersistenceServiceProtocol
    
    init() {
        
    }
    
    var userData: UserData? {
        persistenceService.user
    }
    
    // MARK: - User Interactions
    
    func goToStatistics() {
        onGoToStatistics?()
    }
    
    func goToAchievements() {
        onGoToAchievements?()
    }
    
    func goToSettings() {
        onGoToSettings?()
    }
    
    func goToAbout() {
        onGoToAbout?()
    }
    
    func logOut() {
        confirmationData = .init(title: Localizable.log_out, message: Localizable.log_out_msg, cancelTitle: Localizable.cancel, confirmTitle: Localizable.log_out, action: { [weak self] in
            self?.onLoggedOut?()
            self?.persistenceService.user = nil
        })
    }
}
